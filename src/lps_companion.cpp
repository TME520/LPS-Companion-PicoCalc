/*
 ...
 v1.0 boot and persistent saves confirmed by the user; v1.2 needs device testing.
...
class App {
    ...
    bool commit(const State& candidate){
        if(daysBack){say("Jour archive : lecture seule");return false;}
...
public:
    ...
    void key(int k){
        ...
        if(screen==Screen::Home){
            if(k==Left){
                ...
                else say(state.count?"Plus ancien jour conserve":"Aucun jour archive");
            }
            if(k==Right){daysBack=0;selection=0;page=0;}
            const unsigned menuCount=daysBack?3:4;
            ...
            if(k=='4'&&daysBack)say("Jour archive : lecture seule");
            ...
            else if(daysBack&&(k==Enter||(k>='1'&&k<='9')))say("Jour archive : lecture seule");
            ...
            if(daysBack){if(k==Enter)go(Screen::Home);else say("Jour archive : lecture seule");}
    ...
    void render(){
        hw.clear();char b[80];const Day& day=viewedDay();
        std::snprintf(b,sizeof b,"LPS COMPANION v1.2            J%lu",static_cast<unsigned long>(day.number));line(0,b,true);
        std::snprintf(b,sizeof b,"%lu XP acquis",static_cast<unsigned long>(state.xp));line(1,b);
        if(daysBack)line(2,"JOUR ARCHIVE - LECTURE SEULE");
        if(screen==Screen::Home){
            line(3,daysBack?"JOUR TERMINE":"AUJOURD'HUI");
            std::snprintf(b,sizeof b,"%u/11 activites - %lu XP %s",count(day.flags),static_cast<unsigned long>(points(day)),daysBack?"valides":"a valider");line(4,b);
            constexpr const char* menu[]={"1  Activites","2  Note du jour","3  Poids du jour","4  Terminer le jour"};
            for(unsigned i=0;i<(daysBack?3u:4u);++i)line(6+int(i)*2,menu[i],i==selection);
            line(15,"<- Jour precedent   -> Jour actuel");
        }else if(screen==Screen::Activities){
            std::snprintf(b,sizeof b,"ACTIVITES                       %u/3",page+1);line(3,b);
            unsigned end=(page+1)*PageSize;if(end>ActivityCount)end=ActivityCount;for(unsigned i=page*5;i<end;++i){std::snprintf(b,sizeof b,"%u  %-24s [%c]",i+1,Activities[i],day.flags&(1u<<i)?'x':' ');line(5+int(i-page*5)*2,b,i-page*5==selection);}
            line(16,daysBack?"<- -> pages   Echap: menu du jour":"Entree: cocher  1-9: racc.  <- -> pages");
        }else if(screen==Screen::Note){
            line(3,"NOTE DU JOUR");wrap(5,draft.data(),3);std::snprintf(b,sizeof b,"%zu/96 caracteres",std::strlen(draft.data()));line(10,b);line(14,daysBack?"Entree / Echap : menu du jour":"Entree : valider / Echap : annuler");
        }else if(screen==Screen::Weight){
            ...
            line(14,"Vide = non renseigne");line(15,daysBack?"Entree / Echap : menu du jour":"Haut/Bas : champ - Entree : valider");
        }else if(screen==Screen::Finish){
